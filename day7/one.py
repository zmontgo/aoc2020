...
dirty = """pale cyan bags contain 2 posh black bags, 4 wavy gold bags, 2 vibrant brown bags.
...
for bag in clean:
    contains = (' '.join(bag[4:])).replace(',','').replace('.','').replace('bags','').replace('bag', '')
    contains = re.sub('[0-9]+', '', contains)
    contains = contains.split(' ')
    contains = [i for i in contains if i]
    contains = [ ''.join(x) for x in zip(contains[0::2], contains[1::2]) ]
    bags.append({bag[0] + bag[1]:contains})
 
for bag in bags:
    for subbag in bag.items():
        for subsubbag in subbag[1]:
            if (subsubbag in pointers.keys()):
                pointers[subsubbag].append(subbag[0])
            else:
                pointers[subsubbag] = [subbag[0]]
...
for check in tocheck:
  if check in pointers.keys():
    for add in pointers[check]:
      if add not in true:
        true.append(add)

    for toAdd in pointers[check]:
